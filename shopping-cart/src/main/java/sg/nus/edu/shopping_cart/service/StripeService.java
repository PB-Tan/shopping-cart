...
import sg.nus.edu.shopping_cart.model.Cart;
...
public class StripeService {
        ...
        public StripeResponse payProducts(List<CartItem> cartItems, Cart cart, Order order) {
                ...
                for (CartItem cartItem : cartItems) {
                        ...
                        Long amount = Math.round(cartItem.getUnitPrice() * 100); // need to change double to long for
                                                                                 // api
                        Long quantity = (long) cartItem.getQuantity(); // need to change quantity to long as well

                        // Product name
                        SessionCreateParams.LineItem.PriceData.ProductData productData = SessionCreateParams.LineItem.PriceData.ProductData
                                        .builder()
                                        .setName(cartItem.getProduct().getName()).build();
